import React, { Component } from 'react';
import { Link } from 'react-router-dom';

class Project extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [
                {
                    id: 1,
                    srcimg: 'assets/img/project/runwal-greens-tower.jpg',
                    heading: 'RUNWAL GREEN TOWER',
                    meta: 'Architecture',
                    text: 'Building'
                },
                {
                    id: 2,
                    srcimg: 'assets/img/project/auris-serenity.jpg',
                    heading: 'AURIS SERENITY TOWERS',
                    meta: 'Architecture',
                    text: 'Building'
                },
                {
                    id: 3,
                    srcimg: 'assets/img/project/skycity-tower.jpg',
                    heading: 'SKYCIT TOWER',
                    meta: 'Architecture',
                    text: 'Building'
                },
                {
                    id: 4,
                    srcimg: 'assets/img/project/sarova.jpg',
                    heading: 'SAROVA SHAPOORJI',
                    meta: 'Architecture',
                    text: 'Building'
                },
                {
                    id: 5,
                    srcimg: 'assets/img/project/phoenix-palladium.jpg',
                    heading: 'PHOENIX PALLADIUM',
                    meta: 'Architecture',
                    text: 'Building'
                },
                // {
                //     id: 6,
                //     srcimg: 'assets/img/project/skycity.jpg',
                //     heading: 'SKYCITY MALL',
                //     meta: 'Architecture',
                //     text: 'Building'
                // },
                // {
                //     id: 7,
                //     srcimg: 'assets/img/project/project-3-440x280.jpg',
                //     heading: 'LAKE MEADOWS APARTMENTS',
                //     meta: 'Architecture',
                //     text: 'Building'
                // },
                // {
                //     id: 8,
                //     srcimg: 'assets/img/project/project-4-440x280.jpg',
                //     heading: 'LAKE MEADOWS APARTMENTS',
                //     meta: 'Architecture',
                //     text: 'Building'
                // }
            ]
        }
    }
    render() {
        return (
            <div className="themesflat-carousel-box clearfix" data-gap={30} data-column={4} data-column2={2} data-column3={1}
                data-auto="true" data-loop="true">
                <div className="owl-carousel owl-theme">
                    {
                        this.state.data.map(data => (
                            <div className="themesflat-project style-1 data-effect  clearfix" key={data.id}>
                                <div className="project-item">
                                    <div className="inner">
                                        <div className="thumb data-effect-item has-effect-icon w40 offset-v-43 offset-h-46">
                                            <img src={data.srcimg} alt="altimage" style={{ width: '100%', height: '280px', objectFit: 'cover' }} />
                                            <div className="text-wrap text-center">
                                                <h5 className="heading"><Link to="#">{data.heading}</Link></h5>
                                                <div className="elm-meta">
                                                    <span>
                                                        <Link to="#">{data.meta}</Link></span>
                                                    <span>
                                                        <Link to="#">{data.text}</Link></span>
                                                </div>
                                            </div>
                                            <div className="elm-link">
                                                <Link to="#" className="icon-1 icon-search"></Link>
                                                <Link to="#" className="icon-1"></Link>
                                            </div>
                                            <div className="overlay-effect bg-color-3" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ))
                    }
                </div>
            </div>
        );
    }
}

export default Project;